...
export function computeDCA(
    ...
    data.push({ year: 0, balance: Math.round(balance), principal: Math.round(principal), coasting: false });
    ...
    for (let y = 1; y <= totalYears; y++) {
        ...
        data.push({ year: y, balance: Math.round(balance), principal: Math.round(principal), coasting: isCoasting });
    }
