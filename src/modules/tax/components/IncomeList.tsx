...
export function IncomeList({ items, symbol, toDisplay, toTHB, onChange }: IncomeListProps) {
    const { t } = useLanguage();
    ...
    function updateAmount(id: string, displayVal: string) {
        const thb = displayVal === "" ? 0 : Math.round(toTHB(parseFloat(displayVal) || 0));
        onChange(items.map((item) => (item.id === id ? { ...item, amountTHB: thb } : item)));
    }
            ...
            {items.map((item, idx) => (
                <Stack key={item.id} direction="row" spacing={1} sx={{ alignItems: "flex-start" }}>
                    ...
                    <TextField
                        ...
                        type="number"
                        value={item.amountTHB === 0 ? "" : String(Math.round(toDisplay(item.amountTHB)))}
            ...
            <Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between" }}>
                ...
                {items.length > 1 && (
                    <Typography variant="body2" color="text.secondary">
                        {t("taxIncomeTotal")}: <strong>{symbol} {Math.round(toDisplay(total)).toLocaleString()}</strong>
                    </Typography>
                )}
            </Stack>
        </Stack>
