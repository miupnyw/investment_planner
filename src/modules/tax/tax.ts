...
export function computeTax(inputs: TaxInputs): TaxResult {
  ...
  const annualIncome = incomeItems.reduce(
    (sum, item) => sum + item.amountTHB,
    0,
  );
  ...
  const childAllowance =
    cap(numChildren, 99) * 30_000 + cap(numChildren2ndPlus, 99) * 30_000;
  ...
  const healthInsuranceDed = cap(
    healthInsurance,
    Math.min(25_000, 100_000 - lifeInsuranceDed),
  );
  // Pension can fill the unused life-insurance cap (up to 100,000) plus its own 200,000 cap
  const pensionInsuranceDed = cap(
    pensionInsurance,
    Math.min(annualIncome * 0.15, 300_000 - lifeInsuranceDed),
  );
